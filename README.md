...
*   **Безпека (Zero-Trust):** Захищена мобільна картка авторизації, зберігання токенів у `sessionStorage`, передача через заголовки `X-Admin-Token` / `Bearer`, маскування секретів у відповідях, логування лише sha256-відбитків та захист від LFI/SSRF.
*   **Універсальне збереження:** Надійна валідація схем конфігурації (`AdminConfigRequest`) із підтримкою як плаского, так і вкладеного JSON.
...
*   🟡 **Жовтий рівень (Warning):** Підвищена небезпека, загроза ударних БПЛА (дронів/шахедів) чи тактичної авіації.
*   🔴 **Червоний рівень (Active):** Безпосередня повітряна тривога, ракетна небезпека, швидкісні цілі.
*   🟢 **Відбій (Clear):** Автоматична фіксація відбою окремих рівнів або повної відміни небезпеки з точним розрахунком тривалості за кожною загрозою окремо.
*   🏙 **Ізоляція тривог для Києва:** Моніторинг та сповіщення ізольовані виключно для міста Київ (`м. Київ`, UID 31), повністю виключаючи хибні спрацьовування від тривог у Київській області.
...
Графічні звіти, live dashboard та Telegram використовують чотири незалежні канали:
* **Факт** — суцільна смуга; стан світла: teal «є» (`💡`) та rose «немає» (`⚡️`).
...
* **Семантичні іконки:** Лаконічні та зрозумілі символи в інтерфейсі та повідомленнях (`💡`/`⚡️`/`🟡`/`🔴`/`🟢`) з автоматичною зворотно-сумісною міграцією legacy-налаштувань.

> **Сумісність:** report statistics зберігають legacy fallback розкладу без metadata, щоб не змінювати історичну математику; live dashboard показує `Невідомо`, якщо `schedule_known` відсутній або false.
...
Унікальний алгоритм, що мінімізує «інформаційний шум». Система автоматично переходить у стан спокою, якщо за останні 24 години не було відключень, а в планах на завтра немає обмежень (або режим активовано примусово через адмінку).
* **Повне глушіння реактивних сповіщень:** У режимі спокою бот пригнічує реактивні сповіщення про зміну стану мережі, а також **повністю глушить усі сповіщення про повітряні тривоги** в Telegram-каналі, щоб не турбувати мешканців чату під час стабільної роботи.
* **Гарантована доставка підсумків:** Навіть у Quiet Mode гарантовано доставляється фінальний підсумок доби та щопонеділковий тижневий графічний звіт (керується через `telegram_daily_reports` та `telegram_weekly_reports`) із транзакційною персистентністю (`report_delivery_state.json`) та захистом від дублювання.
