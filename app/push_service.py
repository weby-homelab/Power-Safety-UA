...
VAPID_CLAIMS = {"sub": "mailto:" + os.environ.get('VAPID_CONTACT_EMAIL', 'rekvizitor.ua@gmail.com')}

# Storage path
from app.light_service import DATA_DIR
...
def send_push_notification(title: str, body: str, url: str = '/'):
    if not VAPID_PRIVATE_KEY or not VAPID_PUBLIC_KEY:
        logger.warning("push_vapid_keys_missing", msg="Web Push disabled: VAPID keys not configured")
        return 0
    ...
    for sub in subs:
        try:
            ...
            if e.response and e.response.status_code in (404, 410):
                ...
                logger.info("push_subscription_expired", endpoint=sub.get('endpoint', '')[:50])
            else:
                logger.error("push_send_error", error=str(e), status=getattr(e.response, 'status_code', None))
    ...
    logger.info("push_notifications_sent", sent=sent, total=len(subs), expired=len(expired))
    return sent
