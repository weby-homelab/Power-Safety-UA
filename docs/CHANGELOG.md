...
- **Параметризація образу в Compose (Immutable Compose Image Parameter):** Уніфіковано `docker-compose.yml` з підтримкою змінної `${POWER_SAFETY_IMAGE:-webyhomelab/power-safety-ua:latest}` для детермінованого закріплення digest на STAGING та PRODUCTION. / Parameterized docker-compose services with POWER_SAFETY_IMAGE for deterministic pinning.
