...
def test_load_schedule_slots_with_schedule(mock_get_config, mock_file, mock_exists):
    ...
    mock_schedule = {"yasno": {"G1": {"2026-04-06": {"slots": [True] * 48}}}}
    ...
    events = [
        {
            "timestamp": datetime.datetime(
                2026, 4, 6, 10, 0, tzinfo=KYIV_TZ
            ).timestamp(),
            "event": "down",
        },
        {
            "timestamp": datetime.datetime(
                2026, 4, 6, 12, 0, tzinfo=KYIV_TZ
            ).timestamp(),
            "event": "up",
        },
    ]
...
def test_generate_chart(mock_savefig):
    ...
    day_start = datetime.datetime.combine(date, datetime.time.min).replace(
        tzinfo=KYIV_TZ
    )
    intervals = [
        (day_start, day_start + datetime.timedelta(hours=10), "up"),
        (
            day_start + datetime.timedelta(hours=10),
            day_start + datetime.timedelta(hours=12),
            "down",
        ),
        (
            day_start + datetime.timedelta(hours=12),
            day_start + datetime.timedelta(hours=24),
            "up",
        ),
    ]

    schedule_intervals = [(day_start, 24.0, True)]

    filename, t_up, t_down = generate_chart(
        date, intervals, schedule_intervals, [], theme="dark"
    )
...
def test_generate_chart_en(mock_savefig):
    ...
    filename, t_up, t_down = generate_chart(
        date, intervals, schedule_intervals, [], theme="dark", lang="en"
    )
...
def test_generate_chart_filters_future_aqi(mock_savefig, mock_get):
    ...
    mock_get.return_value.json.return_value = {"hourly": {"pm2_5": [10.0] * 24}}
